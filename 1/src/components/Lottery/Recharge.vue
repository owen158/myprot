    ...
    <div class='Recharge' ref="BackG">
        ...
        <el-container v-if="bankcard.judgment != false">
            <el-main v-if="bankcard.PaymentList !=''">
                <p v-if="bankcard.judgment === true" class="A-main-Title">存款金额（*范围 {{RadioId.min}}-{{RadioId.man}} 元）</p>
                    ...
                    <div v-if="bankcard.type ==='5'">
                        <el-form-item label="" prop="region" v-if="(typeof RadioId.paymentName=='string') != true">
                            <el-select v-model="ruleForm.region" placeholder="请选择银行">
                                <el-option v-for=" v in RadioId.paymentName" :label="v.title" :value="v.bankcode"></el-option>
                            </el-select>
                        </el-form-item>
                    </div>
...
export default {
  ...
  mounted: function() {
    ...
    this.setBackground();
    if (vm.bankcard.type === "") {
      vm.$router.push({ path: vm.withdraw });
    } else {
      this.$nextTick(function() {
        switch (vm.bankcard.type) {
          case "6":
            this.$store.dispatch("inceCloseNew", { id: 3, text: "支付宝" });
            break;
          case "7":
            this.$store.dispatch("inceCloseNew", { id: 3, text: "微信" });
            break;
          case "8":
            this.$store.dispatch("inceCloseNew", { id: 3, text: "财付通" });
            break;
          default:
            this.$store.dispatch("inceCloseNew", { id: 3, text: "网银" });
        }
      });
    }
  },
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(function(valid) {
        if (valid) {
          if (
            vm.ruleForm.acounmt < vm.RadioId.min ||
            vm.ruleForm.acounmt > vm.RadioId.man
          ) {
            let text =
          ...
          if (vm.bankcard.type === "5") {
            ...
            if (vm.RadioId.paymentName === "AK") {
              ...
              one =
                "?bankcode= H5_ONLINE_BANK_PAY&mobile=mobile&acounmt=" +
                vm.ruleForm.acounmt +
                "&" +
                "payId=" +
                vm.RadioId.id;
            ...
            if (
              vm.bankcard.PaymentList[vm.picked].paymentName === "XQ" ||
              vm.bankcard.PaymentList[vm.picked].paymentName === "GC"
            ) {
              src = vm.userNew.jrg + "PlatformPay/onlineBanking";
              one =
                "?bankcode=" +
            ...
            if (Arr != null) {
              ...
              if (pay != null) {
                vm.bankcode.mobile = pay;
              } else {
                vm.bankcode.mobile = undefined;
              }
            }
