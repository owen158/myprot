		...
		success:function  (data) {
			...
			if (ad_code>-1) {
				...
				if (data[ad_code].length == 1) {
					$('#service-right').css('display','none');
					$('#service-left img').attr('src',data[ad_code][0].img1);
					adData.push(data[ad_code][0].img1);
				}
				if(data[ad_code].length == 2){
					$('#service-left img').attr('src',data[ad_code][0].img1);
					$('#service-right img').attr('src',data[ad_code][1].img1);
...
//end-----------------
			...
			$('#_promo_list').empty();
			$('.list-image').empty();
             //    $('.list-image li .wrapper_tabbox_img').text(text);
				...
				//将获取到的数据封装进对象
					...
					// var text = '<img src="'+data[promo_code][i].img1+'" width="98" height="202"/>'
					var tent = '<li><div class="wrapper_tabbox_img"><img width="986" height="156" src="'+data[promo_code][i].img1+'" alt=""></div><div class="wrapper_tabbox_content"><img src="'+data[promo_code][i].img2+'" alt=""></div></li>'
				...
				// 	'</li> ';
				$('.list-image').append(tent);
				// $('#_promo_list').append(promo_li);
			}
			//将对象存入cookie
			setCookie('promo',promoData,1);  //设置公告cookie
			$('.list-image .wrapper_tabbox_img').on('click', function() {
				$(this).next().toggle();
			})
			...
			//有cookie数据的情况下
				...
				if (adCookies!='') {
					...
					if (adArr.length == 1) {
					$('#service-left img').attr('src',adArr[0]);
					$('#service-right').css('display','none');
					}
					if (adArr.length == 2) {
					$('#service-left img').attr('src',adArr[0]);
					$('#service-right img').attr('src',adArr[1]);
					}
				}
				...
				for (var i = 0; i < allP.length; i++) {
					var tent='<li><div class="wrapper_tabbox_img"><img width="986px" height="156px;" src="'+allP[i][0]+'" alt=""></div><div class="wrapper_tabbox_content"><img src="'+allP[i][1]+'" alt=""></div></li>'
                    $('.list-image').append(tent);
					}
					$('.list-image .wrapper_tabbox_img').on('click', function() {
						$(this).next().toggle();
			...
			$.ajax({
				...
				success:function  (data) {
					if (data.length>0) {
						...
						for (var i = 0; i < data.length; i++) {
							a =a +(i+1)+'.'+data[i].rmk+"。"+" ";
							// var marquee_li = '<a href="href="javascript: void(0)"" target="_blank">'+(i+1)+'.&nbsp;'+data[i].rmk+'，详情可查阅' +'</a>';
							// $('.marqueen .js-marquee').append(marquee_li);
						}
						$('.js-marquee a').text(a);
