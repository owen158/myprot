      ...
      <el-radio-group style="display: block;margin-top: 0px;padding-top: 10px" v-model="radio2" size="small">
        <el-row class="el-solid">
          ...
          <el-col :xs="8" :sm="6" :md="3" :lg="2" :xl="2" v-if="bankcard.PaymentList.length>0">
            <el-button class="tutorialB" border @click="jumpTutorial('银联扫码')">支付教程</el-button>
          </el-col>
        </el-row>
        <div class="line-w"></div>
...
export default {
  ...
  methods: {
    ...
    submit: function() {
      ...
      vm.Amount.mobile = "mobile";
      if ("WF" == payName) {
        //WF接onlineBanking接口
        let src = vm.$store.state.userNew.jrg + "PlatformPay/onlineBanking";
        let one =
